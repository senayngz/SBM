...
<body>
	...
	<form:form action="update" method="post" modelAttribute="register">
	
	<h3>Kişisel Bilgiler:</h3>
	 <label>Adınız ve Soyadınız:</label>
   		 <form:input type="text" path="fullName" value="${fullName}"/>
   		 <br>
 	...
 	<label>Telefon Numarası:</label>
 		 <form:input type="text"  path="phoneNo" value="${phoneNo}"/>
 		 <br>
	 ...
	 <label>TC Kimlik Numarası:</label>
   		 <form:input type="text"  path="TCKN" value="${TCKN}"/>
 	...
 	<label>Araç Plakası:</label>
 		 <form:input type="text" id="plateNo" path="plateNo" value="${plateNo}"/>
 		 <br>
 	...
 	<label>Model:</label>
 		 <form:select path="model" value="${model}">
		...
		<br>
		 ...
		 <br>
   		 <br>
   <label>Marka:</label>
	 <form:select path="brand" value="${brand}">
	 ...
	 <label>Şasi Numarası:</label>
   		 <form:input type="text"  path="chassisNo" value="${chassisNo}"/>
   		 <br>
 	...
 	<label>Motor Numarası:</label>
 		 <form:input type="text"  path="engineNo" value="${engineNo}"/>
 		 <br>
	 ...
	 <label>Engelli Aracı mı?:</label>
   		 <form:select path="disabled" value="${disabled}">
 	...
 	<label>Araç Sınıfı:</label>
 		 <form:select path="vehicleGrade" value="${vehicleGrade}">
   	 ...
   	 <label>Kullanım Amacı:</label>
   		 <form:select path="intendedUse" value="${intendedUse}">
 	...
 	<label>Şehir Numarası:</label>
   		 <form:select path="provinceNo" value="${provinceNo}">
	...
	<h3>Risk Bilgisi:</h3>
 	...
 	<label>Basamak Sayısı:</label>
 		 <form:select path="scaleNo" value="${scaleNo}">
			  <form:option value="1">1</form:option>	
	...
	<form:form action="delete" modelAttribute="register" onsubmit="return confirm('Bu işlemin geri dönüşü yoktur. Devam etmek istediğinize emin misiniz?');">
		
		 <form:input type="hidden" path="TCKN" value="${TCKN}"/>
		
		
		<input type="submit" value="Bu Kişiyi Sil">
